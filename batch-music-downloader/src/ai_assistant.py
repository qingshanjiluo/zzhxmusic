...
# 【修复】排行榜键名 → musicdl 音源 client 名映射
# 用于排行榜下载时限制只使用对应音源
CHART_TO_SOURCE_CLIENT = {
    'qq_hot': 'QQMusicClient',
    'qq_new': 'QQMusicClient',
    'qq_pop': 'QQMusicClient',
    'qq_mainland': 'QQMusicClient',
    'qq_europe': 'QQMusicClient',
    'qq_korea': 'QQMusicClient',
    'netease_hot': 'NeteaseMusicClient',
    'netease_new': 'NeteaseMusicClient',
    'netease_soar': 'NeteaseMusicClient',
    'netease_orig': 'NeteaseMusicClient',
}
...
class AIAssistant:
    ...
    def execute_task(self, task_goal: str, options: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            if not no_download and discovered:
                ...
                # 【修复】排行榜任务：根据榜单类型确定 restrict_source
                restrict_source = None
                if intent.get('charts'):
                    chart_key = intent['charts'][0]
                    restrict_source = CHART_TO_SOURCE_CLIENT.get(chart_key)
                    if restrict_source:
                        print(f"  [排行榜限制] 从 {chart_key} 推断仅使用 {restrict_source}")
    ...
    def _execute_download(self, songs: List[Dict],
                          ...
                          no_lyrics: bool = True,
                          no_cover: bool = True) -> Dict:
        """执行下载
        【修复】支持 restrict_source 参数，限制仅使用指定音源
              支持 no_lyrics/no_cover 控制是否下载歌词和封面
        """
        if not songs:
            return {}
        ...
        if no_lyrics:
            print(f"  [配置] 不下载歌词（使用 --lyrics 启用）")
        if no_cover:
            print(f"  [配置] 不下载封面（使用 --cover 启用）")

        # 【修复】若有限源要求，设置 downloader.restrict_source
        if restrict_source:
            downloader.restrict_source = restrict_source
            print(f"  [限源] 仅使用 {restrict_source} 搜索和下载")
